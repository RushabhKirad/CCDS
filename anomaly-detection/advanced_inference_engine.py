...
class AdvancedInferenceEngine:
    ...
    def predict_anomaly(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Hybrid Detection: If high-confidence signature match, force anomaly
            if attack_confidence > 0.85:
                is_anomaly = True
                # Boost confidence if it was low but signature matched
                if confidence < 0.6:
                    confidence = 0.95
    ...
    def _classify_advanced_attack_type(self, request_data: Dict[str, Any], 
        ...
        # Use full payload if available, otherwise path
        payload = request_data.get('payload', request_data.get('path', ''))
        payload = unquote(str(payload)).lower()
        
        user_agent = str(request_data.get('user_agent', '')).lower()
        method = request_data.get('method', 'GET')
        
        # 1. Log4Shell / JNDI Injection (Critical)
        if any(pattern in payload for pattern in ['${jndi:', 'jndi:ldap', 'jndi:dns', 'jndi:rmi']):
            return ('Log4Shell Exploit', 0.99)
            
        # 2. Advanced Scanner Detection
        ...
        if any(pattern in payload for pattern in ['__proto__', '.prototype.', 'constructor']):
            return ('Prototype Pollution', 0.95)
            
        # 4. SQL Injection
        if any(pattern in payload for pattern in ['union', 'select', 'drop', "' or '", "'=''", '--', 'insert', 'delete', 'update', 'information_schema']):
            return ('SQL Injection', 0.92)
        
        # 5. XSS Attack
        if any(pattern in payload for pattern in ['<script', 'javascript:', 'alert(', 'onerror=', '<iframe', 'onload=', 'onclick=', 'document.cookie']):
            return ('XSS Attack', 0.88)
        
        # 6. Command Injection
        if any(pattern in payload for pattern in ['|', '&&', ';', '$(', '`', 'cat ', 'ls ', 'wget ', 'curl ', 'nc ', 'whoami']):
            return ('Command Injection', 0.90)
        
        # 7. Directory Traversal
        if any(pattern in payload for pattern in ['../', '..\\', '%2e%2e', '%252e', '....///', '..%2f', '..%5c']):
            return ('Directory Traversal', 0.85)
        
        # 8. XML Injection / XXE
        if any(pattern in payload for pattern in ['<!entity', '<!doctype', 'system "', 'public "', '&xxe;', 'file:///']):
            return ('XXE Attack', 0.90)
        
        # 9. LDAP Injection
        if any(pattern in payload for pattern in ['*)(', '*)(&', '*))%00', '*()|', '*)(cn=*']):
            return ('LDAP Injection', 0.87)
        
        # 10. NoSQL Injection
        if any(pattern in payload for pattern in ['$ne', '$gt', '$where', '$regex', '[$gt]', '{"$ne":', '[$where]']):
            return ('NoSQL Injection', 0.86)
        
        # 11. SSRF Attack (Cloud Metadata)
        if any(pattern in payload for pattern in ['169.254.169.254', 'metadata.google', 'metadata.azure', '127.0.0.1', '0.0.0.0']):
            return ('SSRF Attack', 0.90)
        
        # 12. File Upload Attack
        if any(pattern in payload for pattern in ['.php', '.jsp', '.asp', '.exe', '.sh', '.py', '.pl', '.rb']):
            return ('File Upload Attack', 0.82)
        
        # 13. Brute Force
        if method == 'POST' and any(pattern in payload for pattern in ['login', 'auth', 'signin', 'admin']):
            return ('Brute Force', 0.70)
        
        # 14. Generic Bot (lower priority)
        ...
        # Logic Flaw / Business Logic Abuse
        if 'price' in payload and '-' in payload:
             return ('Business Logic Abuse', 0.85)
