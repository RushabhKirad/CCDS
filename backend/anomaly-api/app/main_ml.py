...
_last_whitelist_load = 0

def load_whitelist():
    """Reload whitelist from file if changed"""
    global _whitelist_cache, _last_whitelist_load
    try:
        if os.path.exists(WHITELIST_FILE):
            mtime = os.path.getmtime(WHITELIST_FILE)
            if mtime > _last_whitelist_load:
                with open(WHITELIST_FILE, 'r') as f:
                    _whitelist_cache = set(json.load(f))
                _last_whitelist_load = mtime
                print(f"Loaded whitelist with {len(_whitelist_cache)} paths")
    except Exception as e:
...
def is_whitelisted_path(path):
    ...
    load_whitelist() # Check for updates
    if path in _whitelist_cache:
        return True
...
async def predict_anomaly(request: Request):
    ...
    try:
        ...
        # ⛔ FIREWALL CHECK: Block if IP is blacklisted (Active Defense)
        client_ip = request.client.host
        if request.headers.get("X-Forwarded-For"):
            client_ip = request.headers.get("X-Forwarded-For").split(",")[0]
            
        if firewall.is_blocked(client_ip):
            print(f"🚫 BLOCKED: Request from banned IP {client_ip} rejected immediately.")
            return {
                "event_id": f"blocked_{int(datetime.now().timestamp())}",
                "is_anomaly": True,
                "confidence": 1.0,
                "attack_type": "Blocked IP (Active Defense)",
                "method": "Firewall",
                "risk_score": 100,
                "source_ip": client_ip,
                "action": "blocked"
        ...
        # Construct comprehensive payload for analysis
        full_payload = event_data.get("path", "")
        if event_data.get("query"):
            full_payload += "?" + event_data.get("query", "")
        if event_data.get("body"):
            full_payload += " " + str(event_data.get("body", ""))
            
        # Update event data with full payload
        event_data["payload"] = full_payload
        ...
        if ml_state.engine and ml_state.available:
            ...
            try:
                ...
                # 🛡️ ACTIVE DEFENSE: Auto-block high confidence attacks
                if response_data["is_anomaly"] and response_data["confidence"] > 0.90:
                    firewall.block_ip(response_data["source_ip"], duration_seconds=300, reason=response_data["attack_type"])
                    response_data["action"] = "ip_blocked"
            except Exception as e:
...
async def get_blocked_ips():
    """Get list of active blocked IPs"""
    try:
        data = firewall.get_blocked_ips()
        # Convert to list of dicts {ip, ttl}
        return [{"ip": k, "ttl": v} for k, v in data.items()]
    except Exception as e:
        return {"error": str(e)}
